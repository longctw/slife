...
function load_data(columns, sorts) {
	var uri = 'list';
	console.log(preUri + "*********" + uri);
	if (preUri){
		uri = preUri + uri;
	}
    load_data_list(columns, sorts, uri)
}
 ...
 * @param action
 */
function dt_action2(title, action, width, height) {
    // iframe层
    layer.open({
        type: 2,
        title: title,
 ...
 * @returns {string}
 */
function formatDateToStr(now,fmt) {
    var year = now.getFullYear();
    var month = now.getMonth()+1;
    var date = now.getDate();
    var hour = now.getHours();
    ...
    return fmt.replace('yyyy', year).replace('MM', month).replace('dd', date).replace('hh', hour).replace('mm', minute).replace('mi', minute).replace('ss', second);
}